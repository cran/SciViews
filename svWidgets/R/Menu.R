...
function(menu, ...) {
	res <- switch(MenuType(menu),
		winMenu = if (isRgui() && require(utils)) winMenuAdd(menu),
...
function(menu, item, action, image = "", accel = "", options = "") {
	res <- switch(MenuType(menu),
		winMenu = if (isRgui() && require(utils)) {
			winMenuAddItem(menu, item, action);
			if (options == 'state = "disable"') winMenuStateItem(menu, item, FALSE) },
...
function(menu) {
    res <- switch(MenuType(menu),
		winMenu = if (isRgui() && require(utils)) winMenuDel(menu),
...
function(menu, item) {
    res <- switch(MenuType(menu),
		winMenu = if (isRgui() && require(utils)) winMenuDelItem(menu, item),
...
function() {
	res <- character(0)
	if (isRgui() && require(utils)) res <- winMenuNames()
	...
	res <- c(res, names(getTemp(".gui.Menus")))
...
function(menu) {
    res <- switch(MenuType(menu),
		winMenu = if (isRgui() && require(utils)) winMenuItems(menu),
...
function(file = "Menus.txt") {
	...
	dat <- rep("", N)
	L <- data.frame(menu = I(dat), item = I(dat), image = I(dat), accel = I(dat),
