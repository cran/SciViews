...
"guiDlgDir" <- function(title = "Select a directory", dir = getwd(), new = TRUE,
    ...
    res <- tkchooseDirectory(initialdir = dir, mustexist = !new, title = title)
    # Bug: in case of spaces in the directory, the Tcl/Tk dialog box returns
    # a list of items, using space as separator
    res <- paste(res, collapse = " ")
    # Bug: if new == FALSE and the user indicated a a new dir, and then clicked
...
"guiDlgInput" <- function(message = "Enter a value", title = "Input",
    ...
    .res.inbox <<- character(0)# Result is temporary stored in this variable
    inbox <- tktoplevel()
    tktitle(inbox) <- title
