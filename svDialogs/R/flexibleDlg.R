...
"guiPane.tcltk" <- function(dlg, item, ...) {
    ...
    # Check arguments
    if (!inherits(dlg, "guiDialog"))
        stop("'dlg' must be a 'guiDialog' object!")
...
"guiDlg" <- function(title = "Input", message = NULL, help = NULL, sep = NULL,
    width = 50, labelwidth = 0, panes = list(list(type = "entry", message = "Enter data:", default = NULL)), GUI = getOption("guiWidgets")) {
...
"guiDlgFunction" <- function(fun, template = NULL, maxargs = 7, var = "res", width = 40,
    ...
    # Done in NAMESPACE
    #require(svMisc) # For findhtmlhelp()
    Help <- eval(parse(text = paste("findhtmlhelp('", fun, "')", sep = "")))
    if (Help == "") Help <- NULL else Help <- paste('browseURL(findhtmlhelp("', fun, '"))', sep = "") # Help <- paste("help('", fun, "')", sep = "")
...
"guiEval" <- function(code, ident = "GUI ") {
    ...
    if (ident != "") Prompt <- paste(Prompt, ident, Prompt, sep = "")
        cat(Prompt, code[1], "\n", sep = "")
    if (length(code) > 1) {
...
"display.guiDlg" <- function(x, parent = 0, GUI = getOption("guiWidgets"),
    ...
    if (!is.null(GUI) && GUI != "tcltk") { # Custom GUI widgets
        # Look for a guiDlg.<GUI> function
        fun <- paste("guiDlg", GUI, sep=".")
